...
int main(int argc, char* argv[])
{
	...
	Ptr<FeatureDetector> detector = FeatureDetector::create("ORB");//создали детектор
	Ptr<DescriptorExtractor> descriptorExtractor = DescriptorExtractor::create("ORB");//дескриптор
	...
	vector<Point2f> object_keypoint(matches.size());
	vector<Point2f> scene_keypoint(matches.size());

	for (int matchInd = 0; matchInd < matches.size(); matchInd++)
	{
		KeyPoint point_object = keypoints1[matches[matchInd].queryIdx];
		KeyPoint point_scene = keypoints2[matches[matchInd].trainIdx];

		float distance = (float) norm((Point)point_object.pt - center);
		float scale = point_object.size/point_scene.size;

		float angle = atan((center.y-point_object.pt.y) / (center.x-point_object.pt.x));
		float delta_angle = (point_scene.angle - point_object.angle) * CV_PI / 180.0f;

		float scene_dist = distance * scale;
		float scene_angle = angle + delta_angle;

		Point ceter_point_voite = Point((cvRound(point_scene.pt.y+std::cos(scene_angle)*scene_dist))/splitSet,
			(cvRound(point_scene.pt.x+std::sin(scene_angle)*scene_dist))/splitSet);

		hgt.at<int>(ceter_point_voite.x, ceter_point_voite.y)++;

		object_keypoint[matchInd] = keypoints1[matches[matchInd].queryIdx].pt;
		scene_keypoint[matchInd] = keypoints2[matches[matchInd].trainIdx].pt;
	}
	...
	Mat hommat = findHomography(object_keypoint, scene_keypoint, CV_RANSAC);

	warpPerspective(image, hommat, hommat, scene.size());
	imshow("Homography", hommat);
